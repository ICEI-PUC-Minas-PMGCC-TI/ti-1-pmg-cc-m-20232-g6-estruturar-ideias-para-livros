...
function historiaNull() {
    alert("Não há nenhuma história com o ID selecionado. Favor re-entrar na história a partir da página princpal.")
}
...
function salvarEvento(semAlerta) {
...
function atualizarEvento(semAlerta) {
    const JSONConstruido = construirJSON();
...
function adicionarConExterna(idExterno, tipoExterno) {
    ...
    fetch(search)
        .then(response => response.json())
        .then(data => {
            novoValor = data.con_eventos;
            novoValor.push(parseInt(idEvento));
            novoJSON = editarJSON(data, "con_eventos", novoValor);
...
function removerConExterna(idExterno, tipoExterno) {
    ...
    fetch(search)
        ...
        .then(data => {
            novoValor = data.con_eventos;
            index = novoValor.indexOf(idEvento);
            if (index != -1) {
